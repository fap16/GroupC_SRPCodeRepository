...
### This script is for recreation of figure 2A from the paper using the recreated (pre-2016) pipeline analysis data
# INPUTS:  camp_organoid_matrix_V2.csv
# OUTPUTS: outputs/recreated_fig1C_cell_clusters.csv
#          outputs/recreated_fig1C_cluster_markers_top40.csv
#          outputs/recreated_fig1C_seurat_object.rds
...
## Loading the recreated data matrix
message("Loading recreated matrix...")
full_data <- read.csv("camp_organoid_matrix_V2.csv",
                      header      = TRUE,
                      row.names   = 1,
                      check.names = FALSE)
...
## Creating the Seurat object by using FPKM values directly as counts (Seurat normalisation)

message("Creating Seurat object...")
seu <- CreateSeuratObject(counts       = fetal_data,
                          project      = "recreated_fetal",
                          min.cells    = 3,
                          min.features = 200)
...
## Clustering the cells at a resolution of 2.0
...
# Manual assignment for recreated data — automatic BP detection
# Assignment based on bp_index ranking: cluster 4 highest (14.01), cluster 6 second (2.46), cluster 7 third (1.87), cluster 3 fourth (1.69)
score_df$dominant2 <- case_when(
  score_df$cluster == "2" ~ "N",   # clear N winner n_score=1.25
  score_df$cluster == "4" ~ "BP",  # bp_index=14.01, highest BP signal
  TRUE ~ "AP"
)

message("Dominant lineage per cluster:")
print(score_df[, c("cluster","dominant2","ap_score","bp_score",
                   "n_score","bp_index")])
...
write.csv(cluster_map, "outputs/recreated_fig1C_cell_clusters.csv",
          row.names = FALSE)
message("Saved outputs/recreated_fig1C_cell_clusters.csv")

message("Finding cluster markers...")
markers <- FindAllMarkers(seu, only.pos = TRUE,
                          min.pct = 0.20, logfc.threshold = 0.25)
write.csv(markers, "outputs/recreated_fig1C_cluster_markers_all.csv",
          row.names = FALSE)
...
top_markers <- markers %>%
  ...
  slice_head(n = 40) %>%
  ungroup()
write.csv(top_markers, "outputs/recreated_fig1C_cluster_markers_top40.csv",
          row.names = FALSE)
message("Saved outputs/recreated_fig1C_cluster_markers_top40.csv")

saveRDS(seu, "outputs/recreated_fig1C_seurat_object.rds")
message("Outputs:")
message("  outputs/recreated_fig1C_cell_clusters.csv")
message("  outputs/recreated_fig1C_cluster_markers_top40.csv")
